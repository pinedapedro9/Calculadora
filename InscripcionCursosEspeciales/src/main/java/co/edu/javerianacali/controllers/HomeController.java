    ...
    public ModelAndView consulta(HttpServletRequest request) {
     ...
     public ModelAndView filtro(HttpServletRequest request)//se realiza la consulta por usuario del profesor y se envia la lista de profesores en este caso devuelve solo un registro, a metodo de filtro
     {
          try {
            ...
            ModelAndView mav = new ModelAndView();
            mav.addObject("mensaje", "La sesión ha expirado, intentelo de nuevo en filtro.");
            mav.addObject("isError", true);
            mav.setViewName("error");
            return mav;
        }
    ...
    public ModelAndView paso1C(HttpServletRequest request) {
        try {
            ...
            if (numeroCursos > 0) {
                ...
                String mensaje = "<div class='Titulos'>El usuario  no tiene cursos asignados en registro académico para el periodo " + descPeriodo + ".</div><br />";
                mensaje += "<div class='contenidos'>Para mayor información comuníquese con Javevirtual <br />Tel. 3218200 ext. 8490 <br />Correo javevirtual@javerianacali.edu.co</div>";
                mav.addObject("mensaje", mensaje);
                mav.addObject("isError", false);
                mav.setViewName("error");
